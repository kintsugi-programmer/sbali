...
const FloatingDockMobile = ({
  ...
  return (
    <div
      className={cn(
        "fixed bottom-28 right-4 block md:hidden",
        className
      )}
    >
      <AnimatePresence>
        {open && (
          <motion.div
            layoutId="nav"
            className="absolute right-full mr-2 bottom-0 flex flex-col gap-2"
          >
            {items.map((item, idx) => (
              <motion.div
                key={item.title}
      ...
      >
        <IconLayoutNavbarCollapse className="h-10 w-10 text-green-500" />
      </button>
    </div>
  );
};
        ...
        <AnimatePresence>
          {hovered && (
            <motion.div
              ...
              className="px-2 py-0.5 whitespace-pre rounded-md  border bg-neutral-800 border-neutral-900 text-greem-500 absolute left-1/2 -translate-x-1/2 -top-8 w-fit text-xs"
            >
              {title}
            </motion.div>
          )}
